    ...
    return {
      pprogram: function(){
        if (this.program == '' && index == '0') {
          ...
          return "14";
        } else if (this.program == 'youthsave') {
          return "15";
        };
      },
...
var StaffCollectionView = Marionette.CollectionView.extend({
  itemView: staffView,
  initialize: function(){
    this.collection.filterBy('staff-filter', function (model) {
      return _.contains(model.get('roles'), 'staff');
    });
...
var BoardCollectionView = Marionette.CollectionView.extend({
  ...
  initialize: function(){
    this.collection.filterBy('board-filter', function (model) {
      return _.contains(model.get('roles'), 'board');
    });
  },
  appendHtml: function(collectionView, itemView, index){
...
var FellowCollectionView = Marionette.CollectionView.extend({
  ...
  initialize: function(){
    this.collection.filterBy('fellow-filter', function (model) {
      return _.contains(model.get('roles'), 'fellow');
    });
  },
...
// -_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_
// -_-PROGRAM FELLOW SECTION-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
// -_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_

var programfellowView = Marionette.ItemView.extend({
  className: 'expert-container',
  template: '#programfellow',
});

var ProgramFellowCollectionView = Marionette.CollectionView.extend({
  itemView: programfellowView,

  initialize: function(){
    this.collection.filterBy('programfellow-filter', function (model) {
      return _.contains(model.get('roles'), 'programfellow');
    });
  },
...
var CouncilCollectionView = Marionette.CollectionView.extend({
  ...
  initialize: function(){
    this.collection.filterBy('council-filter', function (model) {
      return _.contains(model.get('roles'), 'council');
    });
  },
...
var ObscuraControls = Marionette.ItemView.extend({
  ...
  filterOnType: _.debounce(function(ev) {
    ...
    if (pfp.getFilteredLength() == '0' ) {
      $('.programfellow').addClass( "hidden" );
    } else {
      $('.programfellow').removeClass( "hidden" );
    }
...
App.addRegions({
  ...
  programfellowRegion: '#programfellowList'
});
...
var pfp = new Backbone.Obscura(proxy);
...
App.addInitializer(function(options) {
  ...
  var programfellowRegion = new ProgramFellowCollectionView({
    collection: pfp
  });
  ...
  App.programfellowRegion.show(programfellowRegion);
});
